...
// Solves the power flow using a backward-forward sweep method.
...
// Performs oriented ordering to buses and branches.
//
// Orders the branches by the principle of oriented ordering:
// indices of sending nodes are smaller than the indices of the
...
fn _order_radial(_mpc: &mut MPC) {
    todo!("order_radial")
    /*

    // Initialize
    let slack = mpc.bus(mpc.bus(:,BUS_TYPE) == REF, 1);
